...
class forest_metrics:
	...
	def _output_metrics(self, met, cols, isfloat, ls, isint):
		_cols_x = [''] + cols + ['QA(U)' if not isint else 'NUM(C)']
		_cols_y = [''] + cols + ['QA(P)' if not isint else 'NUM(L)']

		ls.append('\t'.join(_cols_x))
		for _row in xrange(met.shape[0]):
			ls.append('\t'.join([_cols_y[_row + 1]] + [self._format_output_val(met, _row, _col, isfloat) for _col in xrange(met.shape[1])]))

	def output_metrics(self, ls):
		ls.append('')
		ls.append('num of samples')
		self._output_metrics(self.met_num, self.columns, True, ls, True)

		ls.append('')
		ls.append('accuracy metrics')
		self._output_metrics(self.met_wet, self.columns, False, ls, False)
...
def cal_forest_agree_metric(f, tile, year, ls, ks, loose):
	...
	for _r in csv_util.open(f):
		...
		_v_ref = _r.getint('f_y%s' % year)
		_v_val = _r.getint('f_f%s' % year)
		...
		if _v_val < 0 or _v_ref < 0:
			# skip -1
			continue

		if _v_ref not in _vvr:
			_vvr.append(_v_ref)
		if _v_val not in _vvs:
			_vvs.append(_v_val)
...
def cal_forest_change_agree_metric(f, tile, year1, year2, ls, ks, loose):
	...
	for _r in csv_util.open(f):
		...
		if any([_v < 0 for _v in [_p_val1, _p_ref1, _p_val2, _p_ref2]]):
			# skip -1
			continue
		...
		_val_p1 = _convert_pos(_p_val1)
		_val_p2 = _convert_pos(_p_val2)

		_ref_p1 = _convert_pos(_p_ref1)
		_ref_p2 = _convert_pos(_p_ref2)
...
def main():
	...
	if _opts.auto_tile:
		...
		for _tile in _tiles:
			...
			for _y in _years:
				cal_forest_agree_metric(_opts.input, _tile, _y, _ls, _ks, _opts.loose)

			for _y in xrange(len(_years) - 1):
				cal_forest_change_agree_metric(_opts.input, _tile, _years[_y], _years[_y + 1], _ls, _ks, _opts.loose)
		...
		for _y in _years:
			cal_forest_agree_metric(_opts.input, _opts.tile, _y, _ls, _ks, _opts.loose)

		for _y in xrange(len(_years) - 1):
			cal_forest_change_agree_metric(_opts.input, _opts.tile, _years[_y], _years[_y + 1], _ls, _ks, _opts.loose)
...
def _usage():
	...
	_p.add_argument('--loose', dest='loose', default=False, action='store_true')
