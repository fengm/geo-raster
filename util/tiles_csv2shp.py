...
def csv2shapefile(f_csv, f_out, col, f_landsat):
    ...
    _output_geom = _landsat_geom
    
    # if _landsat_geom != _output_geom:
    #     print('Geometry', _landsat_geom, _output_geom)
    #     raise Exception('only support single geometry shapefile')
...
def main(opts):
    ...
    if not _f_ref:
        ...
        elif opts.tile_tag == 'mgrs':
            _f_ref = os.environ['D_DATA_MGRS']
            _col = 'Name'
        else:
    ...
    with file_unzip.file_unzip() as _zip:
        ...
        csv2shapefile(file_mag.get(opts.input).get(), _f_out, _col, _f_ref)
...
def usage():
    ...
    _p.add_argument('--tile-column', dest='column')
