...
def main():
	...
	for i in xrange(len(_hosts)):
		...
		for _l in _o[0].splitlines()[1:]:
			if _opts.command in _l and 'ssh ' not in _l and 'sh_kill.py' not in _l:
				_vs = re.split('\s+', _l)
				_ts.append(_vs[1])
		...
		_cmd = 'ssh %s kill -9 %s &' % (_host, ' '.join(_ts))
		subprocess.Popen(_cmd, shell=True)
		# _p = subprocess.Popen(_cmd, shell=True)
		# _p.communicate()
